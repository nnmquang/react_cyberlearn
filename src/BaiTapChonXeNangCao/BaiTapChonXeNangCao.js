...
export default class BaiTapChonXeNangCao extends Component {
    ...
    //Tạo ra state chưa thông tin thay đổi
    state = {
        carCurrent: {
            "id": 4,
            "title": "Rallye Red",
    ...
    //Viết phương thức xử lý đổi xe bằng cách set lại giá trị mới cho carCurrent từ carItem
    changeCar = (newCar) => {
          this.setState({
              carCurrent:newCar
          })
    ...
    renderWheels = () => {
        return dataWheels.map((item, index) => {
            return <div onClick={()=>{this.changeWheel(item)}} style={{cursor:'pointer'}} className="row mt-1 border border-color-default m-3" key={index}>
                <div className="col-2">
                    <img className='p-3' style={{ width: '100%' }} src={item.img} alt={index} />
                </div>
    ...
    componentDidMount = () => {
        //Đây là phương thức có sẵn của component tự động thực thi sau khi render được gọi, lưu ý : componenDidMount chỉ chạy 
        //một lần đầu tiên sau khi render thực thi
    ...
    componentDidUpdate = () => {
        //Hàm chạy sau khi state thay đổi (Tự kích hoạt sau render)
                    ...
                    <div className="col-6">
                        <div className="model1">
                            <div
                                ...
                                data-amount="8"
                            ></div>
                            {/* <img style={{width:'100%'}} src="./images/images-black/images-black-1/civic-1.jpg" alt='civic-1.jpg' /> */}
                        </div>
                        <div id="appendScript">
